...
const SVG = (ref) => {
  ...
  if (selectAll("svg").empty()) {
    select(ref).append("svg").attr("width", WIDTH).attr("height", HEIGHT);
  }
};
...
const Bar = (ref_radio, barData, yAttribute, xAttribute, totalPopulation) => {
  ...
  bars
    ...
    .on("mouseover", function (d, i) {
      if (yAttribute == "amount") {
        ...
        select(this).style("opacity", 0.7); //opacity of the bars
      } else {
        tooltip
          .html(
            `<div>${toTitle(xAttribute)}: ${d.key}</div>
  ...
  select(ref_radio).selectAll("input").on("click", sort);
  ...
  function sort(arg) {
    if (typeof arg == "string") {
      ...
      var action = select(this).node().value;
    ...
    if (action == "height") {
      const new_data = barData
        .slice()
        .sort((a, b) => ascending(b.value[yAttribute], a.value[yAttribute]));
      ...
      if (barData[0].key.match("\\d+")) {
        var new_data = barData
          .slice()
          .sort((a, b) => ascending(parseInt(a.key), parseInt(b.key)));
      } else {
        var new_data = barData.slice().sort((a, b) => ascending(a.key, b.key));
      }
      change_data(new_data, duration);
      sort_status = "x";
    }
